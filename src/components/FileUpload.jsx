...
export default function FileUpload({ token, conversationId = null, onAnalysis }) {
  ...
  async function handleUpload(selectedFile) {
    ...
    setLoading(true)
    try {
      const res = await uploadFile(token, selectedFile, conversationId)
  ...
  return (
    <div className="flex items-center justify-start gap-3 mt-3">
      <label htmlFor="file-upload" title="Upload file" className="flex items-center gap-2 p-2 bg-white/6 hover:bg-white/10 rounded cursor-pointer text-sm">
        ...
        <span className="text-white/80">{loading ? 'Uploading…' : file ? file.name : 'Upload'}</span>
      </label>
      <input id="file-upload" type="file" className="hidden" disabled={loading} onChange={(e) => { const f = e.target.files?.[0] ?? null; if (f) handleUpload(f) }} />
    </div>
  )
}